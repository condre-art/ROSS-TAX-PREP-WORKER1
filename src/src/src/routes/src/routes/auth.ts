...
export async function authRoute(req: Request, env: Env) {
  ...
  if (user.mfa_enabled) {
    ...
    if (user.mfa_method === "totp") {
      ...
      mfaValid = decryptedSecret && totp.check(mfa_code, decryptedSecret);
...
export async function mfaVerifyRoute(req: Request, env: Env) {
  ...
  if (!totp.check(code, decryptedSecret)) return new Response(JSON.stringify({ error: "Invalid code" }), { status: 401 });
