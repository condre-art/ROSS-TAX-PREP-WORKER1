...
export async function onRequestPost(context) {
  ...
  // Trigger e-file return preparation workflow
  const clientId = crypto.randomUUID();
  try {
    // Post to worker backend to initiate e-file return creation
    const workerUrl = env.WORKER_API_URL || "https://ross-tax-prep-worker1.condre-art.workers.dev";
    
    await fetch(`${workerUrl}/api/efile/prepare`, {
      method: "POST",
      headers: { 
      ...
      body: JSON.stringify({
        ...
        return_type: service.includes("Business") || service.includes("LLC") || service.includes("Corp") ? "business" : "1040",
        source: "intake_form",
        notes: notes,
        auto_start: true // Automatically create return record and prepare for filing
      })
    });
    
    console.log(`[Intake] Initiated e-file preparation for client ${clientId}`);
  } catch (err) {
    console.error("[Intake] Failed to trigger e-file preparation:", err);
    // Don't fail the intake submission if e-file prep fails
  }
