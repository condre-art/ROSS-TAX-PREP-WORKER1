...
BEGIN
  CREATE TABLE staff (
    ...
    mfa_method NVARCHAR(50), -- 'totp', 'email', 'sms'
    mfa_backup_codes NVARCHAR(MAX), -- JSON array
...
-- LINKED BANKS (Client bank accounts for refund deposits and bank products)
CREATE TABLE IF NOT EXISTS linked_banks (
  ...
  routing_number TEXT NOT NULL,
  routing_verified INTEGER DEFAULT 0,
  account_verified INTEGER DEFAULT 0,
  is_primary INTEGER DEFAULT 0,
  bank_product TEXT CHECK(bank_product IN ('RA', 'EPS', 'SBTPG', 'Refundo')),
...
CREATE INDEX IF NOT EXISTS idx_linked_banks_client ON linked_banks(client_id);
CREATE INDEX IF NOT EXISTS idx_linked_banks_status ON linked_banks(status);
CREATE INDEX IF NOT EXISTS idx_linked_banks_primary ON linked_banks(client_id, is_primary);

-- MONEY MANAGEMENT ACCOUNTS (Digital banking accounts)
CREATE TABLE IF NOT EXISTS money_accounts (
  ...
  account_tier TEXT NOT NULL CHECK(account_tier IN ('basic', 'premium', 'business')),
  account_name TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_money_accounts_client ON money_accounts(client_id);
CREATE INDEX IF NOT EXISTS idx_money_accounts_status ON money_accounts(status);
CREATE INDEX IF NOT EXISTS idx_money_accounts_account_number ON money_accounts(account_number);

-- TRANSACTIONS (Account transaction history)
CREATE TABLE IF NOT EXISTS transactions (
  id TEXT PRIMARY KEY,
  account_id TEXT NOT NULL,
  transaction_type TEXT NOT NULL CHECK(transaction_type IN ('deposit', 'withdrawal', 'transfer', 'p2p', 'card', 'check', 'fee', 'interest')),
  amount REAL NOT NULL,
  balance_after REAL NOT NULL,
  description TEXT NOT NULL,
  category TEXT,
  ...
  p2p_recipient_id TEXT,
  card_last4 TEXT,
  check_number TEXT,
  status TEXT NOT NULL CHECK(status IN ('pending', 'posted', 'declined', 'reversed')),
  posted_at TEXT,
  created_at TEXT DEFAULT CURRENT_TIMESTAMP,
  metadata TEXT, -- JSON metadata
...
CREATE TABLE IF NOT EXISTS p2p_transfers (
  ...
  transfer_type TEXT NOT NULL CHECK(transfer_type IN ('instant', 'standard', 'scheduled')),
  status TEXT NOT NULL CHECK(status IN ('pending', 'processing', 'completed', 'declined', 'cancelled', 'expired')),
  scheduled_date TEXT,
  recurring INTEGER DEFAULT 0,
  recurring_frequency TEXT CHECK(recurring_frequency IN ('daily', 'weekly', 'biweekly', 'monthly')),
  ...
  completed_at TEXT,
  created_at TEXT DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (sender_account_id) REFERENCES money_accounts(id),
  FOREIGN KEY (recipient_account_id) REFERENCES money_accounts(id)
);

CREATE INDEX IF NOT EXISTS idx_p2p_sender ON p2p_transfers(sender_client_id);
CREATE INDEX IF NOT EXISTS idx_p2p_recipient ON p2p_transfers(recipient_client_id);
...
-- MOBILE DEPOSITS (Mobile check deposit processing)
CREATE TABLE IF NOT EXISTS mobile_deposits (
  ...
  duplicate_of TEXT,
  status TEXT NOT NULL CHECK(status IN ('pending', 'processing', 'approved', 'declined', 'cleared', 'returned')),
  decline_reason TEXT,
...
CREATE INDEX IF NOT EXISTS idx_mobile_deposits_account ON mobile_deposits(account_id);
CREATE INDEX IF NOT EXISTS idx_mobile_deposits_client ON mobile_deposits(client_id);
CREATE INDEX IF NOT EXISTS idx_mobile_deposits_status ON mobile_deposits(status);
CREATE INDEX IF NOT EXISTS idx_mobile_deposits_created ON mobile_deposits(created_at);

-- DEBIT CARDS (Visa debit card issuing)
CREATE TABLE IF NOT EXISTS debit_cards (
  ...
  status TEXT NOT NULL CHECK(status IN ('pending', 'active', 'frozen', 'cancelled', 'expired')),
  activation_required INTEGER DEFAULT 0,
  activated_at TEXT,
  daily_limit REAL NOT NULL,
  transaction_limit REAL NOT NULL,
  atm_daily_limit REAL NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_debit_cards_client ON debit_cards(client_id);
CREATE INDEX IF NOT EXISTS idx_debit_cards_status ON debit_cards(status);
CREATE INDEX IF NOT EXISTS idx_debit_cards_last4 ON debit_cards(card_last4);

-- CARD AUTHORIZATIONS (Card transaction authorizations)
...
CREATE INDEX IF NOT EXISTS idx_card_auth_account ON card_authorizations(account_id);
CREATE INDEX IF NOT EXISTS idx_card_auth_status ON card_authorizations(status);
CREATE INDEX IF NOT EXISTS idx_card_auth_created ON card_authorizations(created_at);

-- BIOMETRIC CONSENTS (BIPA-compliant consent tracking)
...
-- BIOMETRIC ENROLLMENTS (Facial recognition enrollment)
CREATE TABLE IF NOT EXISTS biometric_enrollments (
  id TEXT PRIMARY KEY,
  client_id TEXT NOT NULL,
  face_id TEXT NOT NULL,
  enrollment_images TEXT NOT NULL, -- JSON array of image URLs
  face_encoding TEXT,
...
CREATE INDEX IF NOT EXISTS idx_biometric_enrollments_client ON biometric_enrollments(client_id);
CREATE INDEX IF NOT EXISTS idx_biometric_enrollments_status ON biometric_enrollments(status);

-- BIOMETRIC VERIFICATIONS (Facial recognition verification attempts)
CREATE TABLE IF NOT EXISTS biometric_verifications (
  id TEXT PRIMARY KEY,
  client_id TEXT NOT NULL,
  enrollment_id TEXT NOT NULL,
  verification_type TEXT NOT NULL CHECK(verification_type IN ('login', 'transaction', 'settings_change', 'p2p_transfer', 'card_activation')),
...
CREATE INDEX IF NOT EXISTS idx_biometric_verifications_client ON biometric_verifications(client_id);
CREATE INDEX IF NOT EXISTS idx_biometric_verifications_enrollment ON biometric_verifications(enrollment_id);
CREATE INDEX IF NOT EXISTS idx_biometric_verifications_status ON biometric_verifications(status);
CREATE INDEX IF NOT EXISTS idx_biometric_verifications_created ON biometric_verifications(created_at);
...
CREATE TABLE IF NOT EXISTS roles (
  ...
  description TEXT,
  role_type TEXT NOT NULL CHECK(role_type IN ('staff', 'customer', 'preparer', 'system')),
...
-- PERMISSIONS (Individual permissions that can be granted)
CREATE TABLE IF NOT EXISTS permissions (
  id TEXT PRIMARY KEY,
  name TEXT UNIQUE NOT NULL,
  description TEXT,
  category TEXT NOT NULL, -- 'user_mgmt', 'rbac', 'customer_data', 'money', 'card', 'fraud', 'audit', 'system', 'document'
  created_at TEXT DEFAULT CURRENT_TIMESTAMP
);
...
-- ACCESS_AUDIT_LOG (Comprehensive access and action logging)
CREATE TABLE IF NOT EXISTS access_audit_log (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL,
  user_type TEXT NOT NULL CHECK(user_type IN ('staff', 'client', 'anonymous')),
  action TEXT NOT NULL, -- 'login', 'logout', 'view_data', 'create', 'update', 'delete', 'transfer', 'approve'
  resource_type TEXT NOT NULL, -- 'user', 'account', 'transaction', 'card', 'document'
  resource_id TEXT,
  result TEXT CHECK(result IN ('success', 'denied', 'failed')), -- Access result
  deny_reason TEXT, -- Why was access denied/failed
  ip_address TEXT,
  user_agent TEXT,
  device_fingerprint TEXT,
  request_id TEXT,
  entity TEXT, -- What entity was accessed (e.g., customer_id, account_id)
  entity_id TEXT,
  user_role TEXT,
  permission_required TEXT, -- Which permission was checked
  timestamp TEXT DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_audit_user ON access_audit_log(user_id, user_type);
...
CREATE INDEX IF NOT EXISTS idx_audit_entity ON access_audit_log(entity, entity_id);

-- ROLE_CERTIFICATIONS (Periodic access reviews)
CREATE TABLE IF NOT EXISTS role_certifications (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL,
  user_type TEXT NOT NULL CHECK(user_type IN ('staff', 'client')),
  certification_date TEXT DEFAULT CURRENT_TIMESTAMP,
  reviewer_id TEXT NOT NULL,
  reviewer_type TEXT NOT NULL CHECK(reviewer_type IN ('staff', 'system')),
  certification_status TEXT CHECK(certification_status IN ('approved', 'revoked', 'pending_review')),
  notes TEXT,
  expires_at TEXT
);

CREATE INDEX IF NOT EXISTS idx_certifications_user ON role_certifications(user_id, user_type);
CREATE INDEX IF NOT EXISTS idx_certifications_date ON role_certifications(certification_date);
CREATE INDEX IF NOT EXISTS idx_certifications_expires ON role_certifications(expires_at);

-- SoD_VIOLATIONS (Segregation of Duties conflict tracking)
CREATE TABLE IF NOT EXISTS sod_violations (
  ...
  violation_type TEXT NOT NULL, -- 'initiate_and_approve', 'create_and_delete', etc.
  detected_at TEXT DEFAULT CURRENT_TIMESTAMP,
  investigation_status TEXT CHECK(investigation_status IN ('open', 'investigating', 'resolved', 'false_positive')),
  resolution_notes TEXT,
  resolved_at TEXT
);

CREATE INDEX IF NOT EXISTS idx_sod_violations_user ON sod_violations(user_id, user_type);
CREATE INDEX IF NOT EXISTS idx_sod_violations_status ON sod_violations(investigation_status);
CREATE INDEX IF NOT EXISTS idx_sod_violations_detected ON sod_violations(detected_at);
...
-- CLIENT_NOTIFICATIONS (Status updates & alerts)
...
-- CLIENT_DOCUMENTS (Document upload & download)
...
-- MESSAGE_THREADS (Secure client messaging)
...
-- CLIENT_ACTIVITY_LOG (Activity history for clients)
...
-- =====================================================
-- REFUND TRANSFER CENTER
-- =====================================================

-- REFUND_TRANSFERS (Refund transfer tracking)
