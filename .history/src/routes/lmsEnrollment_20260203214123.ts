 ...
 * POST /api/lms/enroll
 * 
 * Submit enrollment application with compliance acknowledgments
 */
lmsEnrollmentRouter.post('/enroll', async (req: any, env: any) => {
  try {
    const body = await req.json();
    
    // Validate required fields (adapted for enrollment form)
    ...
    if (!acks || !acks.policies || !acks.conduct || !acks.accreditation || 
        !acks.financialAid || !acks.identity || !acks.data || !acks.absence) {
      return new Response(
    ...
    // Map program to course pricing
    const programMap = {
      'cert1': { name: 'Tax Professional Certificate I', price: 899.00, duration: 8 },
      'cert2': { name: 'Tax Professional Certificate II', price: 1199.00, duration: 10 },
      'practitioner': { name: 'Tax Practitioner Comprehensive', price: 1499.00, duration: 12 },
      'ea': { name: 'Enrolled Agent Exam Preparation', price: 1999.00, duration: 16 },
      'bundle': { name: 'Tax Professional Bundle', price: 4999.00, duration: 36 },
      'aas': { name: 'Associate of Applied Science - Taxation & Accounting', price: 27500.00, duration: 52 }
    };

    const program = programMap[body.program as keyof typeof programMap];
    if (!program) {
      return new Response(
    ...
    if (existingStudent) {
      ...
      studentId = (studentResult.meta.last_row_id || 0) as number;
    }
    
    // Calculate total cost based on textbook selection
    let textbookCost = 0;
    if (body.textbookFormat === 'physical' || body.textbookFormat === 'ebook') {
      textbookCost = 149.99;
    }
    // If bundled, textbook cost is included in tuition
    ...
    // Store enrollment details with acknowledgments
    await env.DB.prepare(`
