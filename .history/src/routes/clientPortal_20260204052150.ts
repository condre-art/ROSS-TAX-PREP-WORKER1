      ...
      const threads = await db
        .prepare(
          `SELECT t.thread_id, t.subject, t.status, t.created_at, t.updated_at,
                  (SELECT COUNT(*) FROM messages_new m WHERE m.thread_id = t.thread_id AND m.is_read = 0 AND m.sender_id != ?) as unread_count,
                  (SELECT m.message_text FROM messages_new m WHERE m.thread_id = t.thread_id ORDER BY m.created_at DESC LIMIT 1) as last_message
