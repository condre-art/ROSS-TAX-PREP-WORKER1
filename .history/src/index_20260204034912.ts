...
import irsTrackingRouter from "./routes/irsTracking";
import aiAssistantRouter from "./routes/aiAssistant";
import efilePrepRouter from "./routes/efilePrep";
import lmsEnrollmentRouter from "./routes/lmsEnrollment";
import refundAdvantageRouter from "./routes/refundAdvantageAPI";
import form8879Router from "./routes/form8879";
...
// --- Helper Functions ---
/**
 * Extracts path parameters from a URL pathname.
 * Example: /api/fi/123/abc → { param0: "123", param1: "abc" }
 */
function extractPathParams(pathname: string): Record<string, string> {
  const parts = pathname.split("/").filter(Boolean);
  // Remove known prefixes (api, fi, etc.) if needed, or just return numbered params
  const params: Record<string, string> = {};
  // Start after the first two segments (e.g., /api/fi)
  for (let i = 2; i < parts.length; i++) {
    params[`param${i - 2}`] = parts[i];
  }
  return params;
}
...
export default {
  async fetch(req: Request, env: any, ctx: any): Promise<Response> {
    ...
    // --- E-file Preparation API Route (all /api/efile/prepare/* endpoints) ---
    if (url.pathname.startsWith("/api/efile/prepare")) {
      ...
      const resp = await efilePrepRouter.handle(efileReq, env);
      return cors(resp);
    }
    ...
    // --- Refund Advantage Partnership API Route (all /api/bank-products/*, /api/loans/*, /api/webhooks/refund-advantage) ---
    if (
      url.pathname.startsWith("/api/bank-products/") ||
      url.pathname.startsWith("/api/loans/") ||
      url.pathname === "/api/webhooks/refund-advantage"
    ) {
      const refundReq = new Request(url.pathname, req);
      Object.defineProperty(refundReq, "params", {
        value: extractPathParams(url.pathname)
      });
      const resp = await refundAdvantageRouter.handle(refundReq, env);
      return cors(resp);
    }

    // --- Notifications API Route (all /api/notifications/* endpoints) ---
    if (url.pathname.startsWith("/api/notifications")) {
      const notifReq = new Request(url.pathname, req);
      Object.defineProperty(notifReq, "params", { value: {} });
      const resp = await notificationsRouter.handle(notifReq, env);
      return cors(resp);
    }

    // --- IRS Tracking API Route (all /api/irs/* endpoints) ---
    if (url.pathname.startsWith("/api/irs")) {
      const irsReq = new Request(url.pathname, req);
      Object.defineProperty(irsReq, "params", { value: {} });
      const resp = await irsTrackingRouter.handle(irsReq, env);
      return cors(resp);
    }

    // --- Form 8879 API Route (all /api/form-8879/* endpoints) ---
    if (url.pathname.startsWith("/api/form-8879")) {
      const form8879Req = new Request(url.pathname, req);
      Object.defineProperty(form8879Req, "params", {
        value: extractPathParams(url.pathname)
      });
      const resp = await form8879Router.handle(form8879Req, env);
      return cors(resp);
    }

    // --- Financial Institution API Route (all /api/fi/* endpoints) ---
    if (url.pathname.startsWith("/api/fi")) {
      const fiReq = new Request(url.pathname, req);
      Object.defineProperty(fiReq, "params", {
        value: extractPathParams(url.pathname)
      });
      const resp = await fiRouter.handle(fiReq, env);
      return cors(resp);
    }

    // --- Client Bank Management API Route (all /api/client/banks/* endpoints) ---
    if (url.pathname.startsWith("/api/client/banks")) {
      const reqPath = url.pathname.replace(/^\/api/, "");
      const bankReq = new Request(reqPath || "/", req);
      Object.defineProperty(bankReq, "params", {
        value: extractPathParams(url.pathname)
      });
      const resp = await clientBankRouter.handle(bankReq, env);
      return cors(resp);
    }

    // --- Money Management API Route (all /api/money/* endpoints) ---
    if (url.pathname.startsWith("/api/money")) {
      const reqPath = url.pathname.replace(/^\/api/, "");
      const moneyReq = new Request(reqPath || "/", req);
      Object.defineProperty(moneyReq, "params", {
        value: extractPathParams(url.pathname)
      });
    ...
    if (url.pathname === "/api/services/request" && req.method === "POST") {
      const { handleServiceRequest } = await import("./routes/serviceRequests");
      return cors(await handleServiceRequest(req as any, env));
    }

    // GET /api/services/history/:clientId — Get service request history
    if (url.pathname.startsWith("/api/services/history/") && req.method === "GET") {
      const { handleServiceHistory } = await import("./routes/serviceRequests");
      const clientId = url.pathname.split("/").pop();
      const reqWithParams = Object.assign(req, { params: { clientId } });
      return cors(await handleServiceHistory(reqWithParams as any, env));
    }

    // PATCH /api/services/request/:requestId — Update service request (staff only)
    if (url.pathname.startsWith("/api/services/request/") && req.method === "PATCH") {
      const { handleUpdateServiceRequest } = await import("./routes/serviceRequests");
      const requestId = url.pathname.split("/").pop();
      const reqWithParams = Object.assign(req, { params: { requestId } });
      return cors(await handleUpdateServiceRequest(reqWithParams as any, env));
    }

    // POST /api/documents/upload — Upload document for service request
    if (url.pathname === "/api/documents/upload" && req.method === "POST") {
      const { handleDocumentUpload } = await import("./routes/serviceRequests");
      return cors(await handleDocumentUpload(req as any, env));
    }
    ...
    if (url.pathname.startsWith("/api/lms")) {
      // Handle enrollment and certificate endpoints with lmsEnrollmentRouter
      if (url.pathname.startsWith("/api/lms/enroll") || 
          url.pathname.startsWith("/api/lms/enrollments") || 
          url.pathname.startsWith("/api/lms/certificates")) {
        const reqPath = url.pathname.replace(/^\/api\/lms/, "");
        const lmsEnrollReq = new Request(reqPath || "/", req);
        Object.defineProperty(lmsEnrollReq, "params", { value: {} });
        const resp = await lmsEnrollmentRouter.handle(lmsEnrollReq, env);
        return cors(resp);
      }
      
      // Handle other LMS routes (courses, students, etc.) with original lmsRouter
      const reqPath = url.pathname.replace(/^\/api\/lms/, "");
